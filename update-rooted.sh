...
then
	...
	if grep -q tap0 /tmp/vpn.conf 
		then
			...
			echo "vpn.conf is empty"
			#create a new vpn.conf
			REALHOSTNAME2=`find /etc/openvpn/vpn -maxdepth 1 -name "eneco*.crt" | cut -d\/ -f5 | cut -d\. -f1`
			echo "Creating new vpn.conf with hostname: $REALHOSTNAME2"
...
# Empty
EOT
			sed -i "s~blabla~$REALHOSTNAME2~g" /tmp/vpn.conf
